...
//GETS TO POINTS PAGE
  ...
  // Process login form POST and direct to members points page if ok and back to login page if not
app.post("/gen_login.html", function (request, response) {
    ...
    if(typeof users_reg_data[the_username] != 'undefined'){   //To check if the username exists in the json data
      if( users_reg_data[the_username].password ==request.body.password){
               ...
               response.redirect('/gen_ptpg.html?' + `&username=${the_username}`); //Adds username & quantity to invoice
      }
       else {
